...
public:
	...
	void Execute (BaseKernelParams& p, const QTrkComputedConfig& cfg, QI::StreamInstance* s, QI::DeviceInstance* d, device_vec<float3>* initial, device_vec<float3> *output);
...
private:
	template<typename TImageSampler>
	void Iterate(BaseKernelParams& p, device_vec<float3>* initial, device_vec<float3>* output, StreamInstance *s, int angularSteps);
...
template<typename TImageSampler>
__global__ void QI_ComputeProfile(BaseKernelParams kp, float3* positions, float* quadrants, float2* profiles, float2* reverseProfiles, QIParams params)
{
	int idx = threadIdx.x + blockDim.x * blockIdx.x;
	if (idx < count && (kp.locParams[idx].locType&LT_QI)) {
		...
		for (int q=0;q<4;q++) {
			ComputeQuadrantProfile<TImageSampler> (kp.images, idx, &img_qdr[q*params.radialSteps], params, q, 
				make_float2(positions[idx].x, positions[idx].y), kp.imgmeans[idx]);
		}
...
void QI::Execute (BaseKernelParams& p, const QTrkComputedConfig& cfg, QI::StreamInstance* s, QI::DeviceInstance* d, device_vec<float3>* initial, device_vec<float3> *output) 
{
	...
	params.cos_sin_table = d->qi_trigtable.data;

	for (int a=0;a<cfg.qi_iterations;a++) {
		Iterate<TImageSampler> (p, a==0 ? initial : output, output, s, std::max(MIN_RADPROFILE_SMP_COUNT, (int)angsteps) );
		angsteps *= cfg.qi_angstep_factor;
	}
}

template<typename TImageSampler>
void QI::Iterate(BaseKernelParams& p, device_vec<float3>* initial, device_vec<float3>* newpos, StreamInstance *s, int angularSteps)
{
	dim3 qdrThreads(16, 8);

	if (0) {
		dim3 qdrDim( (njobs + qdrThreads.x - 1) / qdrThreads.x, (params.radialSteps + qdrThreads.y - 1) / qdrThreads.y, 4 );
		QI_ComputeQuadrants<TImageSampler> <<< qdrDim , qdrThreads, 0, s->stream >>> 
			(p, initial->data, s->d_quadrants.data, params);

		QI_QuadrantsToProfiles <<< blocks(njobs), threads(), 0, s->stream >>> 
			(s->d_quadrants.data, s->d_QIprofiles.data, s->d_QIprofiles_reverse.data, params);
	}
	else {
		QIParams qiparams(params);
		qiparams.angularSteps = angularSteps;

		QI_ComputeProfile <TImageSampler> <<< blocks(njobs), threads(), 0, s->stream >>> (p, initial->data, 
	...
	QI_OffsetPositions<<<blocks(njobs), threads(), 0, s->stream>>>
		(p, initial->data, newpos->data, prof, qi_FFT_length, d_offsets, pixelsPerProfLen, s->d_shiftbuffer.data); 
}
